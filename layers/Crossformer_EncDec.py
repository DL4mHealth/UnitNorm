...
from utils.tools import get_norm_layer


class SegMerging(nn.Module):
    def __init__(self, d_model, win_size, norm):
        super().__init__()
        self.d_model = d_model
        self.win_size = win_size
        self.linear_trans = nn.Linear(win_size * d_model, d_model)
        self.norm = get_norm_layer(norm, win_size * d_model, batch_norm=nn.BatchNorm2d)
...
class scale_block(nn.Module):
    def __init__(
        ...
        if win_size > 1:
            self.merge_layer = SegMerging(d_model, win_size, norm)
        else:
        ...
        for i in range(depth):
            self.encode_layers.append(
                TwoStageAttentionLayer(
                    configs, seg_num, factor, d_model, n_heads, d_ff, dropout, norm
                )
            )
